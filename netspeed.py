...
def open_networkspeed(parent):
    # Check if 'network_speed_window' already exists and is still open
    ...
    # If it does not exist or has been closed, create a new one
...
def networkspeed_widgets(self):
   ...
   # Configure the top frame to display the title image
    ...
    self.circular_button = customtkinter.CTkButton(
        ...
        ,border_width=5,
        hover_color="#4A39BA",
        command=lambda: start_speedtest_thread(self)  
    )
...
def networkspeed_data(self):
    ...
    self.dspeed_label.configure(text=f"Download Speed: {download_speed:.2f} Mbps")
    self.uspeed_label.configure(text=f"Upload Speed: {upload_speed:.2f} Mbps")
    self.latency_label.configure(text=f"Latency: {latency} ms")
    self.provider_label.configure(text=f"Provider: {server}")
    self.server_label.configure(text=f"Server IP: {server_ip}")



def start_speedtest_thread(self):
    # Create a thread to run the networkspeed_data function
    speedtest_thread = threading.Thread(target=networkspeed_data, args=(self,))
